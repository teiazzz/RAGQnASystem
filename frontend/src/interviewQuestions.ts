export type InterviewDifficulty = '基础' | '中等' | '深挖'
...
export const interviewStages = Array.from(
  new Set(interviewQuestions.map((item) => item.stage)),
)

export const interviewTags = Array.from(
  new Set(interviewQuestions.flatMap((item) => item.tags)),
